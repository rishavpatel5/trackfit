...
function dayMacros(meals: { calories: number; protein: number; carbs: number; fat: number }[]) {
  return meals.reduce(
    (acc, m) => ({
...
export function dietsRouter(env: Env) {
  const r = Router();
  r.use(authMiddleware(env));

  r.get(
    "/clients/:clientId/weeks",
  ...
  r.post(
    ...
    asyncHandler(async (req: AuthedRequest, res) => {
      ...
      if (exists) throw new AppError(409, "Week already exists");
  ...
  r.patch(
    ...
    asyncHandler(async (req: AuthedRequest, res) => {
      ...
      if (!week) throw new AppError(404, "Week not found");
