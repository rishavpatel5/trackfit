...
export function startOfCalendarDay(date = new Date()): Date {
...
export function computeSessionFlags(
  trainerStatus: TrainerAttendanceStatus,
  clientStatus: ClientAttendanceStatus,
): { sessionCompleted: boolean; sessionCharged: boolean } {
...
export async function countChargedSessions(clientId: string, tx?: Prisma.TransactionClient) {
  const db = tx ?? prisma;
...
export async function syncSessionsCompleted(clientId: string, tx?: Prisma.TransactionClient) {
  const db = tx ?? prisma;
