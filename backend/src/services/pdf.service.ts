...
function buildWeightTrajectoryChart(
  measurements: { recordedAt: Date; weight: number | null }[],
): string {
  const points = measurements
    .filter((m): m is { recordedAt: Date; weight: number } => m.weight != null && m.weight > 0)
  ...
  const yAt = (w: number) => pad.top + plotH - ((w - yMin) / (yMax - yMin)) * plotH;
  ...
  const dots = points
    .map(
      (p, i) =>
        `<circle cx="${xAt(i).toFixed(1)}" cy="${yAt(p.weight).toFixed(1)}" r="4" fill="#2563eb" stroke="#fff" stroke-width="1.5"/>`,
    )
    .join("");
...
export async function buildClientReportHtml(prisma: PrismaClient, clientId: string) {
  ...
  const attendanceSummary = client.attendanceRecords.reduce(
    (acc, r) => {
      ...
      if (r.sessionCharged) acc.charged += 1;
  ...
  const progressNotes = client.progressEntries
    .map(
      (p) =>
        `<div style="margin-bottom:10px;padding:10px;background:#fafafa;border-radius:6px"><div><strong>${p.weekStartDate ? formatDateIST(p.weekStartDate) : "Week " + (p.weekNumber ?? "")}</strong></div><div>${escapeHtml(p.trainerComments ?? "")}</div><div>Recovery: ${escapeHtml(p.recovery ?? "")} · Energy: ${escapeHtml(p.energyLevel ?? "")}</div></div>`,
    )
    .join("");
