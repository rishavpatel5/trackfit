...
export async function buildClientReportHtml(prisma, clientId) {
    ...
    const progressNotes = client.progressEntries
        .map((p) => `<div style="margin-bottom:10px;padding:10px;background:#fafafa;border-radius:6px"><div><strong>${p.weekStartDate?.toISOString().slice(0, 10) ?? "Week " + (p.weekNumber ?? "")}</strong></div><div>${escapeHtml(p.trainerComments ?? "")}</div><div>Recovery: ${escapeHtml(p.recovery ?? "")} · Energy: ${escapeHtml(p.energyLevel ?? "")}</div></div>`)
        .join("");
