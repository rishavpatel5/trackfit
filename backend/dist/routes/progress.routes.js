...
export function progressRouter(env) {
    const r = Router();
    r.use(authMiddleware(env));
    r.get("/clients/:clientId/entries", asyncHandler(async (req, res) => {
    ...
    r.post("/clients/:clientId/entries", asyncHandler(async (req, res) => {
    ...
    r.patch("/entries/:id", asyncHandler(async (req, res) => {
        if (req.user.role === "CLIENT")
            throw new AppError(403, "Forbidden");
    ...
    r.get("/clients/:clientId/photos", asyncHandler(async (req, res) => {
        await assertClientAccessInline(req, paramId(req.params, "clientId"));
    ...
    r.post("/clients/:clientId/photos", asyncHandler(async (req, res) => {
        if (req.user.role === "CLIENT")
            throw new AppError(403, "Forbidden");
        await assertClientAccessInline(req, paramId(req.params, "clientId"));
