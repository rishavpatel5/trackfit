...
export type WorkoutWeek = {
  id: string;
  weekNumber: number;
  days: { label: string; sortOrder?: number; exercises: ApiWorkoutExercise[] }[];
};

export function WorkoutPlanSection({ clientId, canEdit }: { clientId: string; canEdit: boolean }) {
  ...
  async function addWeekNum(weekNumber: number, template?: boolean) {
    try {
      if (template) {
        await api.post(`/workouts/clients/${clientId}/weeks`, {
          weekNumber,
          days: [
            {
              label: "Day A — Customize label",
              exercises: [
                { name: "Main lift", sets: 4, reps: "8", weight: "", restSec: 120, tempo: "", notes: "", sortOrder: 0 },
                { name: "Accessory", sets: 3, reps: "12", weight: "", restSec: 90, tempo: "", notes: "", sortOrder: 1 },
              ],
            },
          ],
        });
        ...
        await api.post(`/workouts/clients/${clientId}/weeks`, {
          ...
          days: [
            {
              label: "Day 1 — rename",
              exercises: [{ name: "Exercise name", sets: 3, reps: "", weight: "", restSec: 60, tempo: "", notes: "", sortOrder: 0 }],
            },
          ],
        });
      }
      toast.success(`Week ${weekNumber} added — open Edit to personalize`);
      refresh();
    } catch {
    ...
    <div className="space-y-6">
      {canEdit ? (
        <div className="flex flex-col gap-3 rounded-xl border border-dashed border-border/80 bg-muted/10 p-4 md:flex-row md:flex-wrap md:items-end md:gap-x-4">
          <WorkoutWeekCreateToolbar onSubmit={addWeekNum} />
        </div>
      ) : null}
        ...
        {weeks.map((week) => (
          <AccordionItem key={week.id} value={week.id}>
            <div className="flex flex-wrap items-center justify-between gap-2 pr-4">
              ...
              {canEdit ? (
                <WorkoutWeekEditorDialog week={week} onSaved={refresh}>
                  <Button size="sm" variant="outline" className="shrink-0 border-border">
                    Edit week
                  </Button>
                </WorkoutWeekEditorDialog>
              ) : null}
            </div>
            <AccordionContent>
                  ...
                  <div key={day.label}>
                    ...
                    <ul className="mt-2 space-y-2 text-sm text-muted-foreground">
                      {(day.exercises ?? []).map((ex, i) => (
                        <li key={`${week.id}-${day.label}-${i}`}>
                          <span className="text-foreground">{ex.name}</span>
                          {" · "}
                          {ex.sets != null ? `${ex.sets} sets` : "—"}
                          {" · "}
                          {ex.reps || "—"} reps{item(ex.weight)}
                          {" · "}rest {ex.restSec ?? "—"}s
                          {" · "}tempo {ex.tempo || "—"}
                          {ex.notes ? (
                            <>
                              {" · "}
                              <span className="italic">Note: {ex.notes}</span>
                            </>
                          ) : null}
                        </li>
                      ))}
      ...
      {weeks.length === 0 ? <p className="text-center text-sm text-muted-foreground">No training weeks yet.</p> : null}
    </div>
  );
}

function item(weight: string | null) {
  if (!weight) return "";
  return ` @ ${weight}`;
}

function WorkoutWeekCreateToolbar({ onSubmit }: { onSubmit: (n: number, template?: boolean) => void }) {
  const [n, setN] = useState(1);
  return (
    <>
      <div>
        <Label>Week number</Label>
        <Input type="number" min={1} className="w-28 mt-1" value={n} onChange={(e) => setN(Number(e.target.value))} />
      </div>
      <Button type="button" onClick={() => onSubmit(n, false)}>
        Add custom week (starter rows)
      </Button>
      <Button type="button" variant="secondary" className="bg-muted" onClick={() => onSubmit(n, true)}>
        Add week with split template (edit freely)
      </Button>
    </>
  );
}

function WorkoutWeekEditorDialog({
  week,
  onSaved,
  children,
}: {
    ...
    setDays(
      week.days.map((d, di) => ({
        ...
        exercises: (d.exercises ?? []).map((ex, ei) => ({
          ...
          tempo: ex.tempo ?? "",
          notes: ex.notes ?? "",
          sortOrder: typeof ex.sortOrder === "number" ? ex.sortOrder : ei,
        })),
      })),
    );
  }, [open, week]);
  ...
  async function save() {
    if (days.length === 0) {
      toast.error("Add at least one training day before saving.");
      return;
    }
    for (const d of days) {
      if ((d.exercises ?? []).some((ex) => !ex.name.trim())) {
    ...
    try {
      await api.patch(`/workouts/weeks/${week.id}`, {
        days: days.map((d, di) => ({
          label: d.label,
          sortOrder: di,
          exercises: (d.exercises ?? []).map((ex, ei) => ({
      ...
      toast.success("Workout week saved");
      setOpen(false);
      onSaved();
    } catch {
      toast.error("Save failed — check numeric fields.");
    }
  }
  ...
  function addDay() {
    setDays([
      ...days,
      {
        ...
        exercises: [{ name: "New exercise", sets: 3, reps: "", weight: "", restSec: 90, tempo: "", notes: "", sortOrder: 0 }],
      },
    ]);
  }
  ...
  function addExercise(dayIdx: number) {
    const next = [...days];
    const ex = next[dayIdx].exercises ?? [];
    ex.push({
      name: "",
      sets: 3,
      reps: "",
    ...
    next[dayIdx] = { ...next[dayIdx], exercises: ex };
    setDays(next);
  }
  ...
  function patchExercise(dayIdx: number, exIdx: number, patch: Partial<ApiWorkoutExercise>) {
    const next = [...days];
    const ex = [...(next[dayIdx].exercises ?? [])];
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-h-[92vh] max-w-3xl overflow-y-auto">
        ...
        <div className="space-y-6 pb-10">
          {days.map((day, di) => (
            <Card key={`day-${di}`} className="border-border bg-card/70">
              <CardContent className="space-y-4 pt-4">
                <div className="flex flex-wrap items-end gap-2">
                  <div className="min-w-[200px] flex-1">
                    <Label>Day label</Label>
                    <Input value={day.label} onChange={(e) => updateDayLabel(di, e.target.value)} placeholder="Monday · Strength" />
                  </div>
                  <Button type="button" variant="destructive" size="sm" className="self-end text-xs h-9" onClick={() => removeDay(di)}>
                    Remove day
                  </Button>
                </div>
                <div className="space-y-3">
                  {(day.exercises ?? []).map((ex, ei) => (
                    <Card key={`ex-${di}-${ei}`} className="border-border bg-muted/20 p-4">
                      <div className="grid gap-2 md:grid-cols-12">
                        ...
                        <div className="md:col-span-2">
                          ...
                          <Input
                            ...
                            value={ex.sets ?? ""}
                            onChange={(e) =>
                              patchExercise(di, ei, { sets: e.target.value === "" ? null : Number(e.target.value) })
                            }
                          />
                        </div>
                        <div className="md:col-span-2">
                          <Label>Reps</Label>
                          <Input value={ex.reps ?? ""} onChange={(e) => patchExercise(di, ei, { reps: e.target.value })} />
                        </div>
                        ...
                        <div className="md:col-span-3">
                          ...
                          <Input
                            ...
                            value={ex.restSec ?? ""}
                            onChange={(e) =>
                              patchExercise(di, ei, { restSec: e.target.value === "" ? null : Number(e.target.value) })
                            }
                          />
                        </div>
                        <div className="md:col-span-3">
                          <Label>Tempo</Label>
                          <Input value={ex.tempo ?? ""} onChange={(e) => patchExercise(di, ei, { tempo: e.target.value })} />
                        </div>
                        <div className="md:col-span-12">
                          <Label>Notes</Label>
                          <Input value={ex.notes ?? ""} onChange={(e) => patchExercise(di, ei, { notes: e.target.value })} />
                        </div>
                        <div className="md:col-span-12 flex justify-end">
                          <Button type="button" variant="outline" size="sm" className="text-destructive" onClick={() => removeExercise(di, ei)}>
                            Remove exercise
                          </Button>
                        </div>
                      </div>
                  ...
                  <Button type="button" variant="outline" size="sm" onClick={() => addExercise(di)}>
          ...
          <div className="flex flex-wrap gap-2 border-t border-border pt-4">
            <Button type="button" variant="secondary" className="bg-muted" onClick={addDay}>
              + Add training day
            </Button>
